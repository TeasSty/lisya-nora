...
## Деплой (бесплатный preview на Cloudflare Workers)

В этой рабочей среде нет авторизованного доступа к Cloudflare (`wrangler whoami`
возвращает «Not logged in», интерактивный логин недоступен). Поэтому проект полностью
готов к деплою, но сам деплой должен запустить пользователь — это займёт 5–10 минут:
...
После получения реального адреса замените `DEPLOY_URL_PLACEHOLDER` в `index.html`
(канонический URL и Open Graph теги) на настоящий домен.
