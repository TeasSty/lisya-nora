...
- Панель администратора (`/admin`) доступна и оформлена в стиле сайта.
  ...
  показать заказчице. Пароль демо-входа **не публикуем** в README и на странице
  входа (спросите у разработчика). После подключения сервера/БД поставьте
  `DEMO_MODE = false`.
...
После этого кнопка «Выбрать пункт Ozon» в форме заявки начнёт подтягивать список
ПВЗ по городу и подставлять адрес. Пока ключей нет — остаётся запасной путь через
карту [ozon.ru/info/map](https://www.ozon.ru/info/map/).
